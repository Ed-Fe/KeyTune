...
- **Ativar AutoDJ**: analisa a faixa atual e até seis próximas opções em segundo plano, inclusive em playlists novas, evita repetir artistas recentes e escolhe pela energia local da transição, tonalidade maior/menor, diferença de volume percebido e compatibilidade de tempo. Quando a grade de batidas é confiável, estima o primeiro tempo do compasso e mudanças de seção, alinha frases de quatro compassos, sincroniza o tempo das duas faixas e corrige pequenos desvios de fase durante a sobreposição. O ponto de entrada pode pular uma introdução silenciosa ou fraca; a troca progressiva de graves e médios termina com o corte da faixa anterior na batida planejada. Também pode ser ligado ou desligado em **Reprodução > Ativar AutoDJ**. A fila manual sempre tem prioridade. Se a análise atrasar, falhar ou não tiver confiança suficiente, o player usa o crossfade comum configurado ou segue para a próxima faixa normalmente.
