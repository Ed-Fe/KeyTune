...
class YouTubeMusicDependenciesTests(unittest.TestCase):
    ...
    def test_import_yt_dlp_raises_guidance_when_management_is_disabled(self):
        with patch("player.youtube_music.dependencies.activate_youtube_dependency_target_dir"):
            with patch("player.youtube_music.dependencies.importlib.import_module", side_effect=ImportError("missing")):
                with self.assertRaisesRegex(RuntimeError, "Recursos adicionais"):
                    import_yt_dlp_module()

    def test_import_yt_dlp_attempts_install_when_management_is_enabled(self):
        configure_youtube_dependency_management(
            managed_install_enabled=True,
            auto_update_enabled=True,
        )

        fake_module = types.SimpleNamespace(__version__="2026.1.31")
        with patch("player.youtube_music.dependencies.activate_youtube_dependency_target_dir"):
            with patch.object(youtube_dependencies, "install_or_update_youtube_dependencies") as install_mock:
                with patch.object(
                    ...
                    imported_module = import_yt_dlp_module()

        self.assertIs(imported_module, fake_module)
        install_mock.assert_called_once_with(force=False)

    def test_install_update_skips_command_when_dependencies_are_already_available(self):
        with patch("player.youtube_music.dependencies.activate_youtube_dependency_target_dir"):
    ...
    def test_install_update_runs_pip_with_target_directory(self):
            ...
            with patch("player.youtube_music.dependencies._run_pip_install", return_value=completed_process) as run_mock:
                with patch("player.youtube_music.dependencies.youtube_dependencies_available", return_value=True):
                    with patch(
                        ...
                        result = install_or_update_youtube_dependencies(force=True, timeout_seconds=33)
